...
SWEP.Category				= "CoD Zombies" --The category.  Please, just choose something generic or something I've already done if you plan on only doing like one swep..
...
SWEP.Primary.Ammo = "Wind Reels" -- What kind of ammo.  Options, besides custom, include pistol, 357, smg1, ar2, buckshot, slam, SniperPenetratedRound, and AirboatGun.
...
function SWEP:PrimaryAttack(what)

    local pos = self.Owner:GetShootPos() 
	local ppos = pos + (self.Owner:GetForward() * 250)
	local o = self.Owner
	if !self:CanPrimaryAttack() then
		timer.Simple(0.25, function() self:Reload() end)
		return
	end
...
if SERVER then
	...
	local pos = self.Owner:GetShootPos() + o:GetForward() * 20
	...
	o:ViewPunch( Angle( -5, 0, 0 ))
	BaseClass.PrimaryAttack( self, what ) 
end
...
if SERVER then
		...
		if v == self.Owner then continue end
		 //if gamemode.Get("nzombies") and v == self.Owner or v:IsPlayer() then continue end
	if gamemode.Get("nzombies") and v:IsPlayer() and hook.Run("PlayerShouldTakeDamage",v,self.Owner) then 
	   return 
	end	
	 if gamemode.Get("sandbox") and v:IsPlayer() or v:IsNPC() then
	 local dmginfo = DamageInfo()
	 dmginfo:SetDamage( 9999 ) 
	 dmginfo:SetDamageType( DMG_GENERIC ) 
	 dmginfo:SetAttacker( self.Owner ) 
	 dmginfo:SetDamageForce( (self.Owner:GetForward() * 300000) + (self.Owner:GetUp() * 300000) ) 
	 v:TakeDamageInfo( dmginfo ) 
	 elseif v.Type == "nextbot" then
	 local dmginfo2 = DamageInfo()
	 dmginfo2:SetDamage( 9999 ) 
	 dmginfo2:SetDamageType( DMG_GENERIC ) 
	 dmginfo2:SetAttacker( self.Owner ) 
	 dmginfo2:SetDamageForce( (self.Owner:GetForward() * 30000) + (self.Owner:GetUp() * 30000) ) 
...
if self.Ispackapunched == 0 then
	if self.Weapon:Clip1() == 2 then
		self.Owner:GetViewModel():SetSubMaterial(10, "models/weapons/thundergun/_-gt5_weapon_mesh_gray.vmt")
		self.Owner:GetViewModel():SetSubMaterial(7, "models/weapons/thundergun/_-gt5_weapon_mesh_gray.vmt")
		self.Owner:GetViewModel():SetSubMaterial(9, nil)
		self.Owner:GetViewModel():SetSubMaterial(8, nil)
	elseif self.Weapon:Clip1() == 1 then
		self.Owner:GetViewModel():SetSubMaterial(9, "models/weapons/thundergun/_-gt5_weapon_mesh_gray.vmt")
		self.Owner:GetViewModel():SetSubMaterial(10, "models/weapons/thundergun/_-gt5_weapon_mesh_gray.vmt")
		self.Owner:GetViewModel():SetSubMaterial(7, "models/weapons/thundergun/_-gt5_weapon_mesh_gray.vmt")
		self.Owner:GetViewModel():SetSubMaterial(8, nil)
	elseif self.Weapon:Clip1() == 0 then
		self.Owner:GetViewModel():SetSubMaterial(9, "models/weapons/thundergun/_-gt5_weapon_mesh_gray.vmt")
		self.Owner:GetViewModel():SetSubMaterial(8, "models/weapons/thundergun/_-gt5_weapon_mesh_gray.vmt")
		self.Owner:GetViewModel():SetSubMaterial(10, "models/weapons/thundergun/_-gt5_weapon_mesh_gray.vmt")
		self.Owner:GetViewModel():SetSubMaterial(7, "models/weapons/thundergun/_-gt5_weapon_mesh_gray.vmt")
	end
else
	--self.Owner:GetViewModel():SetSubMaterial(5, "models/XQM/LightLinesRed_tool.vtf")
	self.Owner:GetViewModel():SetSubMaterial(0, "models/XQM/LightLinesRed_tool.vtf")
	if (self.Weapon:Clip1() == 4) then
		self.Owner:GetViewModel():SetSubMaterial(10, nil)
		self.Owner:GetViewModel():SetSubMaterial(9, nil)
		self.Owner:GetViewModel():SetSubMaterial(8, nil)
		self.Owner:GetViewModel():SetSubMaterial(7, nil)
