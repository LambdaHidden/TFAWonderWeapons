...
function SWEP:PrimaryAttack(old)
	...
	if clip > 1 then
		...
		self.Weapon:EmitSound("thundergun/fly_thundergun_shoot.wav")
	elseif clip == 1 then
		self:SendWeaponAnim(ACT_VM_PRIMARYATTACK_EMPTY)
		self.Weapon:EmitSound("thundergun/fly_thundergun_last_shoot.wav")
	end
...
if SERVER then
	...
	local ppos = pos + (self.Owner:GetForward() * 250)
		 local ed = EffectData()
		 ed:SetOrigin(pos)
		 ed:SetStart(pos + self.Owner:GetForward() * 10)
		 ed:SetAttachment(1)
...
if self.Ispackapunched == 0 then
	...
	elseif self.Weapon:Clip1() == 0 then
		...
		--self.Owner:GetViewModel():SetSubMaterial(4, "models/weapons/common/bo1_pap_camo_c.vmt")
